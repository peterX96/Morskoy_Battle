...
class Additional_background():
                ...
                if (event.button == 3):
                    ...
                    if (angle_flag != -1):
                        ...
                        for i in range(size_ship[_ship] + 1):
                            if ((angle_flag == 0 and self.battlefield[N][y][x+i] != 0) or (angle_flag == 1 and self.battlefield[N][y][x+i] != 0)):
                                flag = False
                                break
                        ...
                        if (flag):
                            ...
                            self.flag_angle.append(angle_flag)
                            self.id.append(d)
    ...
    def auto_set_ship(self, N):
        ...
        self.flag = 1
        self.catalog = [0, 0, 0, 0]

    def draw_battleground(self):

        if (screen_id == 2):
            for i in range(0, 400, 100):
                s_f.text(950, 170 + i, str(self.catalog[i//100]), BLACK, 64)
            ...
            for i in range(len(self.id)):
                ...
                if (self.flag_angle[i] == 0):
                    ...
                    _x, x_rect = s_f.rot_center(
                        _x, self.angle[i], center_ship[0], center_ship[1])

                else:
                    center_ship = (x * delta + x0 + delta / 2,
                                   y * delta + half_size_ship + y0)
                    _x, x_rect = s_f.rot_center(
                        _x, 90 + self.angle[i], center_ship[0], center_ship[1])

                x_rect = _x.get_rect(center=center_ship)
                screen.blit(_x, x_rect)

    def continue_button(self):
        return (int(self.catalog == [0, 0, 0, 0]))
...
while not finished:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT or flag_quit == 1:
            finished = True
        elif screen_id != 2 and screen_id != 4:
            ...
            rect(screen, (255, 255, 255), (0, 0, 1000, 1000), 0)
